...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans=3, in_dim=64, dim=96, norm=nn.BatchNorm2d):
        """
        Args:
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, dim,
        ...
        # self.norm1 = FrozenBatchNorm2d(dim, eps=1e-5)
        self.act1 = nn.GELU()
        self.conv2 = nn.Conv2d(dim, dim, kernel_size=kernel_size, stride=1, padding=1)
        self.norm2 = nn.BatchNorm2d(dim, eps=1e-5)
        # self.norm2 = FrozenBatchNorm2d(dim, eps=1e-5)
...
class FasterViT(nn.Module):
    ...
    def __init__(self,
                 ...
                 do_propagation=False,
                #  stem_norm_layer=nn.LayerNorm,
                 stem_norm_layer=nn.BatchNorm2d,
                 output_norm_layer=nn.BatchNorm2d,
                 out_indices=(0, 1, 2, 3),
                 frozen_stages=-1,
                 **kwargs):
        """
        ...
        self.patch_embed = PatchEmbed(in_chans=in_chans, in_dim=in_dim, dim=dim, norm=stem_norm_layer)
        ...
        for i_layer in self.out_indices:
            layer = output_norm_layer(self.num_features[i_layer])
            if i_layer == 3:
                layer_name = f'norm'
            else:
...
def build_fastervit(modelname, stem_norm_layer, output_norm_layer, **kw):
    ...
    model_para_dict = {
        'faster_vit_0_224': dict(
            ...
            output_norm_layer=output_norm_layer,
        ),
        'faster_vit_0_224_hat_False': dict(
            depths=[2, 3, 6, 5],
            num_heads=[2, 4, 8, 16],
            window_size=[7, 7, 7, 7],
            dim=64,
